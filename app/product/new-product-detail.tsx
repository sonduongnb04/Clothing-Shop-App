      ...
      <ScrollView showsVerticalScrollIndicator={false}>
                ...
                renderItem={({ item, index }) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      // Nếu có hình ảnh biến thể, cập nhật hình ảnh chính
                      if (item.image) {
                        const imageIndex = product.images.findIndex(img => img === item.image);
                        if (imageIndex !== -1) {
        ...
        <View style={styles.priceContainer}>
          ...
          {product.installmentInfo && (
            <View style={styles.installmentContainer}>
              <ThemedText style={styles.installmentText}>{product.installmentInfo}</ThemedText>
            </View>
          )}
        </View>
        ...
        {product.colors && product.colors.length > 0 && (
          <View style={styles.optionContainer}>
            <ThemedText style={styles.optionTitle}>Màu sắc:</ThemedText>
            <View style={styles.colorOptions}>
              {product.colors.map((color, index) => (
                ...
                >
                  <ThemedText style={selectedColor === color ? styles.selectedOptionText : styles.optionText}>
                    {color}
                  </ThemedText>
                </TouchableOpacity>
              ))}
